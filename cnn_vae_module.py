...
def decode(iteration, decode_k, sample_num, sample_d, manual, model_dir, agent):
    ...
    with torch.no_grad():
        ...
        save_image(sample_d.view(sample_num, 3, 28, 28),model_dir+'/recon'+agent+'/random_'+str(decode_k)+'.png') if manual != True else save_image(sample_d.view(sample_num, nc, 28, 28),model_dir+'/recon'+agent+'/manual_'+str(decode_k)+'.png')
