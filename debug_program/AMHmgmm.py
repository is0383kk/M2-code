...
K = 3
c_nd_A = np.loadtxt("./data1.txt");c_nd_B = np.loadtxt("./data2.txt");z_truth_n = np.loadtxt("./true_label.txt") 
...
w_dk = np.zeros((D, K)); w_dk_A = np.zeros((D, K)); w_dk_B = np.zeros((D, K))
...
for i in range(iteration):
    ...
    for d in range(D):
        ...
        if judge_r >= rand_u: 
            # 受容
            w_dk[d] = w_dk_A[d] # w_d = w_d^{Sp}
            count_AtoB = count_AtoB + 1 # 受容した回数をカウント
        else: 
            # 棄却
            w_dk[d] = w_dk_B[d] # w_d = w_d^{Li}
    ...
    for k in range(K):
        ...
        # エージェントB：更新後の\mu,\Lambdaからw^Aの事後分布のパラメータを計算
        tmp_eta_nB[k] = np.diag(
            -0.5 * (c_nd_B - mu_kd_B[k]).dot(lambda_kdd_B[k]).dot((c_nd_B - mu_kd_B[k]).T)
        ).copy() 
        tmp_eta_nB[k] += 0.5 * np.log(np.linalg.det(lambda_kdd_B[k]) + 1e-7)
        eta_dkB[:, k] = np.exp(tmp_eta_nB[k])
        # エージェントA：１イテレーション前の\mu,\Lambdaからw^Bの事後分布のパラメータを計算
        tmp_eta_nA[k] = np.diag(
            -0.5 * (c_nd_A - mu_kd_A[k]).dot(lambda_kdd_A[k]).dot((c_nd_A - mu_kd_A[k]).T)
        ).copy() 
        tmp_eta_nA[k] += 0.5 * np.log(np.linalg.det(lambda_kdd_A[k]) + 1e-7)
    ...
    for d in range(D):
        ...
        if judge_r >= rand_u: 
            # 受容
            w_dk[d] = w_dk_B[d] # w_d = w_d^{Sp}
            count_BtoA = count_BtoA + 1 # 受容した回数をカウント
        else: 
            # 棄却
            w_dk[d] = w_dk_A[d] # w_d = w_d^{Li}
