...
def mutual_info(parameter_dir):
    ...
    for c in range(len(phi_n)):
        for w in range(len(phi_n[0])):
            t_t_prob=(phi_n[c][w]*G[c])/(prob_n[w]*G[c])+1e-100
            t_t_prob=(phi_n[c][w]*G[c])*math.log(t_t_prob)

            t_f_prob=((1.0-phi_n[c][w])*G[c])/((1.0-prob_n[w])*G[c])+1e-100
            #print phi_sum[w]
            t_f_prob=((1.0-phi_n[c][w])*G[c])*math.log(t_f_prob)

            f_t_prob=((1-G[c])*(1.0-phi_n[c][w]))/(prob_n[w]*(1-G[c]))+1e-100
            f_t_prob=((1-G[c])*(1.0-phi_n[c][w]))*math.log(f_t_prob)

            f_f_prob=((1.0-G[c])*(1.0-phi_n[c][w]))/((1.0-prob_n[w])*(1.0-G[c]))+1e-100
