...
def word_data_read(diric,DATA_NUM,space_name,DATA_initial_index,word_class,W,word_diric):
    file = glob.glob(diric+"/"+word_diric+"/*")
    convert = lambda text: int(text) if text.isdigit() else text 
    alphanum_key = lambda key: [ convert(c) for c in re.split('([0-9]+)', key) ] 
    file.sort(key=alphanum_key)
    ...
    for i in range(DATA_NUM):
        ...
        signal=(diric+"/"+word_diric+"/word"+repr(i+DATA_initial_index)+".txt") in file
        
        if signal==True:
            word_data_ind.append(i)
            f=open(diric+"/"+word_diric+"/word"+repr(i+DATA_initial_index)+".txt","r")
            data=f.read()
            line=data.split("\n")
