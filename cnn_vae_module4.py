...
class VAE(nn.Module):
    def __init__(self, image_channels=image_channels, h_dim=h_dim, x_dim=x_dim):
        ...
        self.decoder = nn.Sequential(
            UnFlatten(),
            nn.ConvTranspose2d(in_channels=h_dim, out_channels=256, kernel_size=5, stride=2),
            nn.ReLU(),
            ...
            nn.ReLU(),
            nn.ConvTranspose2d(in_channels=64, out_channels=image_channels, kernel_size=6, stride=2),            
            nn.Sigmoid(),
        )
        self.fc1 = nn.Linear(h_dim, x_dim)
        self.fc2 = nn.Linear(h_dim, x_dim)
