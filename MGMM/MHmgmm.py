...
c_nd_A = np.loadtxt("dataA.txt")
c_nd_B = np.loadtxt("dataB.txt")
z_truth_n = np.loadtxt("label.txt")
...
w_dd_A = np.identity(dim) * 0.5; w_dd_B = np.identity(dim) * 0.5 # Hyperparameters for \Lambda^A, \Lambda^B
nu = dim
